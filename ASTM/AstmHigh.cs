...
namespace GbService.ASTM
{
    public class AstmHigh
    {
        ...
        public static void Parse(AstmManager manager, string msg)
        {
            ...
            if (manager.Kind == Jihas.Vitek || msg.Contains("mtrsl"))
            {
              VidasHandler.ParseVitek(msg, manager.Instrument);
                return;
            }
        ...
        {
            // 1. Adjust dates
            start = start.AddDays(-(double)instrument.InstrumentDays);
            end = end.AddDays(-(double)instrument.InstrumentDays);
            ...
            // 3. Build Query using LINQ (Not Stored Procedure)
            IQueryable<Analysis> queryable = from x in db.Analysis
                                             where x.CreatedDate >= start && x.CreatedDate <= end
                                                && (int)x.AnalysisState == (int)state
                                                && (int)x.AnalysisRequest.AnalysisRequestState != 3
                                             select x;

            // 4. Filter by Instrument ID if Std is null
            queryable = ((instrument.InstrumentStd == null) ?
                (from x in queryable where x.InstrumentId == (int?)instrument.InstrumentId select x) :
                queryable);

            AstmHigh._logger.Info(string.Format("count q0= {0}", queryable.Count<Analysis>()));

            // 5. Filter Logic
            IQueryable<Analysis> source = (instrument.Kind == Jihas.Navify && instrument.Mode == 1) ?
                (from x in queryable where x.AnalysisStatusId != x.InstrumentId select x) :
                queryable.Where((Analysis x) => x.AnalysisType.AnalysisTypeInstrumentMappings.Any((AnalysisTypeInstrumentMapping mp) => mp.InstrumentCode == instrument.InstrumentCode && !(mp.AnalysisTypeCode == null || mp.AnalysisTypeCode.Trim() == string.Empty)));

            AstmHigh._logger.Info(string.Format("count q1= {0}", source.Count<Analysis>()));

            // 6. Get Samples
            IQueryable<Sample> queryable2 = from x in (from x in source select x.Sample).Distinct<Sample>()
                                            where x.SampleCode != null
                                            select x;

            // 7. Return List
            List<Sample> list = ((instrumentDataBits == "D") ?
                (from x in queryable2 where x.DateReceived != null select x) :
                queryable2).Take(20).ToList<Sample>();
        ...
        public static List<string> OrderSamples(List<Sample> samplesList, Instrument instrument, ASTM_Message message = null, bool test = false)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // ---------------------------------------------------------
                    // . INDIKO PLUS Order /  (ASTM E1394)
                    // ---------------------------------------------------------
                    else if (kind == Jihas.Indiko)
                    {
                        foreach (var sample in list)
                        {
                            ...
                            // Format Tests: ^^^Test1\^^^Test2
                            string testsString = string.Join("\\", tests.Select(t => "^^^" + t));

                            // Prepare Data
                            string text4 = instrument.Now.ToString("yyyyMMddHHmmss");
                            string barcode = sample.FormattedSampleCode;
                            string priority = sample.AnalysisRequest.IsEmergency ? "S" : "R";
                            var p = sample.AnalysisRequest.Patient;

                            // Build ASTM Message
                            StringBuilder sb = new StringBuilder();

                            // Header
                            sb.Append($"H|\\^&|||LIS|||||Indiko||P|1394-97|{text4}\r");

                            // Patient
                            sb.Append($"P|1|{p.PatientID}|||{p.Nom}^{p.Prenom}||||||||||||||||||||\r");

                            // Order (O|Seq|Barcode||^^^Test\^^^Test|Priority...)
                            sb.Append($"O|1|{barcode}||{testsString}|{priority}|||||||||||||||||||O\r");
                    ...
                    // ---------------------------------------------------------
                    // 6. GENERIC / DEFAULT (Maglumi, ACL, etc.)
                    // ---------------------------------------------------------
                    {
        ...
        private static string EncodeSamplesAutobio(List<Sample> samples, Instrument instrument, string reqDate, string reqCmd)
        {
            ...
            foreach (Sample sample in samples)
            {
                ...
                // 2. Patient Record {p.PatientID}
                msg += $"P|{seq}||{p.PatientID}||{p.Nom}^{p.Prenom}|||{sex}||||||||||||||||gynecology^ward1^bed1|||||||||\r";
                ...
                {
                    // [FIX] Add the '^' here so EVERY test starts with it (Component 2)
                    testFields.Add($"^{t}^1");
                }

                // 2. Join with '\'
                // Result: "^103^1\^104^1"
                string allTests = string.Join("\\", testFields);
                msg += $"O|{oSeq}||^{barcode}^^|{allTests}|{priority}|||||||||{timeToUse}|0|||1|||F|||||||||\r";
                     oSeq++;
        ...
        public static string EncodeSample(Sample sample, Instrument instrument, ASTM_Message message, bool test = false)
        {
            ...
            string textS = text.Contains("urin") ? "3" : (text.Contains("sang") ? "6" : "1");
            bool isEmergency = sample.AnalysisRequest.IsEmergency;
            ...
            string text55 = patient.PatientTelMob ?? "";
            string text56 = patient.PatientNomPrenom ?? "";
            string Medecin = sample.AnalysisRequest.MedecinString;
            ...
            // [NEW] Logic to get Partner Name
            string partnerName = "";
            long? partnerId = sample.AnalysisRequest.PartnerId;

            if (partnerId != null)
            {
                // Try to get from navigation property first
                // If sample.AnalysisRequest.Partner is null, query the DB
                using (var db = new LaboContext())
                {
                    // Assuming your Partner table/entity is named 'Partner' and has a 'Nom' or 'Name' field
                    // You might need to adjust 'Partner' and 'Nom' to match your exact class names
                    var partner = db.Partner.Find(partnerId);
                    if (partner != null)
                    {
                        partnerName = partner.Name; // Or partner.PartnerName
                    }
                }
            }
            ...
            if (flag)
            {
                ...
                else
                {
                    ...
                    {
                        text11 += string.Format(
                            "O|{0}|{1}||{2}|N|{3}|||||||||CENTBLOOD||||||||||X|||||<CR>",
                            i + 1,
                            formattedSampleCode,
                            testsPreffix[i],
                            text4
            ...
            else if (kind == Jihas.Vitek)
            {
                var handler = new VidasHandler(null);
                return handler.EncodeSampleVitek(sample);
            }
            else
            {
                                                            ...
                                                            if (flag14)
                                                            {
                                                                ...
                                                                result = string.Concat(new string[]
                                                                {
                                                                            /*  patient.PatientID,
                                                                              ...
                                                                              patient.ShortSexe*/
                                                                            // "H|^&&|||^LIS host^1.0|||||||P\r",
                                                                     "H|\\^&|||1^LIS host^1.0|||||||P\r",
                                                                    string.Format(
                                                                    "P|1|{0}|||{1}|||{2}||||{4}|||||||||||||{5}|\r", patient.PatientID, text56,patient.ShortSexe,patient.PatientDateNaiss,text55,Medecin),
                                                                    "O|1|",formattedSampleCode,"||",text10,"|",priority,"||",text4, "||||||",partnerName,"||",textS,"||||||||||O\rL|1|F\r"
                                                                });
                                                            }
        ...
        public static JihazResult GetResult(AstmOrder astm, Instrument instrument, char repeater)
        {
            ...
            /*  bool flagUF1000 = kind == Jihas.SysmexUf4000 && instrument.Mode==1;
              if (flagUF1000)
              {
            ...
            if (flag)
            {
                if (!string.IsNullOrEmpty(astm.Instrument))
                {
                    string[] parts = astm.Instrument.Split(new char[] { repeater });

                    // FIX: Check if we actually have enough parts before grabbing index 2
                    if (parts.Length > 2)
                    {
                        text = parts[2];
                    }
